...
def inicio(request):
    libros_todos = libro.objects.all()
    query = request.GET.get('q')
    if query:
        libros_filtrados = libros_todos.filter(
            Q(titulo__icontains=query)
        )
        return render(request, 'inicio.html', {'libros': libros_filtrados, 'query': query})
    else:
        return render(request, 'inicio.html', {'libros': libros_todos})
...
def libros(request):
    ...
    query = request.GET.get('q')
    if query:
        libros_filtrados = libros.filter(
            Q(titulo__icontains=query)
        )
        return render(request, 'libros.html', {'libros': libros_filtrados, 'query': query})
    else:
...
def agregar_comentario(request, id_libro):
    ...
    if request.method == 'POST':
        ...
        form = comentarioform()
        return render(request, 'agregar_comentario.html', {'formulario': form, 'libros': libros, 'id_libro': id_libro})

@login_required(login_url='/iniciarsesion/') 
def editar_comentario(request, id_comentario): 
    comentario_obj = comentario.objects.get(id=id_comentario)
    if request.method == 'POST': 
        form = comentarioform(request.POST, instance=comentario_obj) 
        if form.is_valid(): 
            form.save() 
            messages.success(request, '¡El comentario se ha actualizado correctamente!') 
            return redirect('comentarios', id_libro=comentario_obj.libro.id) 
        else: 
            messages.error(request, 'Por favor complete todos los campos')
    else: 
        form = comentarioform(instance=comentario_obj) 
    return render(request, 'editar_comentario.html', {'formulario': form, 'id_libro': comentario_obj.libro.id}) 
